...
> Working on the distributed voting system gave me a much clearer sense of how distributed systems operate in practice, as opposed to straightforward sequential programs. For this project, we opted to use Supabase rather than Google Cloud Platform, since setting up billing on Google Cloud was a hurdle. I noticed that the system’s components—edge nodes, the API layer, messaging service, worker processes, and database—functioned independently yet coordinated through asynchronous messaging. During normal operation, votes were handled accurately, and in failure scenarios, queued requests were processed once the worker came back online, highlighting the significance of fault tolerance and message buffering. I also observed that repeated vote submissions increased processing demands, but implementing idempotency successfully prevented duplicate entries in the database. One of the main challenges was properly configuring and linking the services, as debugging a distributed system proved far more complex than working with a single program. Overall, this project reinforced my understanding that distributed systems can greatly enhance scalability and reliability, though they also bring added complexity in areas like synchronization, monitoring, and troubleshooting.
